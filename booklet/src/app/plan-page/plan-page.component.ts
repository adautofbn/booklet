    ...
    if (this.plt.is('cordova')) {
      this.pdfObj.getBuffer(buffer => {
        ...
        this.diagnostic.requestExternalStorageAuthorization().then(() => {
          this.file.writeFile(this.file.externalDataDirectory, 'plano_' + plan.title + '.pdf', binaryArray, { replace: true })
          .then(fileEntry => {
            this.fileOpener.open(this.file.externalDataDirectory + 'plano_' + plan.title + '.pdf', 'application/pdf');
          });
        }).catch(error => {
          this.alertService.error('Permissão necessária não foi concedida ao aplicativo');
        })
